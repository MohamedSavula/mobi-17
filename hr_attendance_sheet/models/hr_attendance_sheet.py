...
class attendance_sheet(models.Model):
    ...
    no_travel = fields.Integer(string="No of Travel Allowance", required=False, store=True)
    tot_travel = fields.Integer(string="Total Travel Allowance", required=False, store=True)
    previous_absence = fields.Integer(string="Previous Absence", required=False, store=True)
    current_absence = fields.Integer(string="Current Absence", required=False, store=True,
                                     compute='get_current_absence')
    tot_current_absence_hours = fields.Integer(string="Total Current Absence Hours", required=False, store=True,
                                     compute='get_current_absence')
    next_absence = fields.Integer(string="Next Absence", required=False, store=True, compute='get_current_absence')

    # @api.model
    # def create(self, vals):
    #     res = super(attendance_sheet, self).create(vals)
    #     for rec in res:
    #         sheet = self.env['attendance.sheet'].sudo().search(
    #             [('date_to', '=', rec.date_from - relativedelta(days=1)), ('employee_id', '=', rec.employee_id.id)], limit=1)
    #         print('sheet',sheet)
    #         rec.previous_absence=sheet.next_absence
    #     return res

    
    @api.depends('no_absence', 'previous_absence')
    def get_current_absence(self):
        for rec in self:
            rec.current_absence = rec.no_absence
            rec.tot_current_absence_hours=rec.tot_current_absence_hours
            rec.next_absence = rec.next_absence
            if rec.no_absence + rec.previous_absence > 5:
                rec.current_absence = 5
                rec.next_absence = rec.no_absence + rec.previous_absence - 5
                rec.tot_current_absence_hours=rec.next_absence*rec.employee_id.resource_calendar_id.hours_per_day
            else:
                rec.current_absence = rec.no_absence + rec.previous_absence
                rec.tot_current_absence_hours=rec.next_absence*rec.employee_id.resource_calendar_id.hours_per_day
                rec.next_absence = 0
    ...
    def action_attsheet_confirm_all(self):
        for rec in self:
    ...
    def action_attsheet_confirm(self):
        ...
        for rec in self:
            sheet = self.env['attendance.sheet'].sudo().search(
                [('date_to', '=', rec.date_from - relativedelta(days=1)), ('employee_id', '=', rec.employee_id.id), ('state', '=', 'done')], limit=1)
            print('sheet',sheet)
            rec.previous_absence=sheet.next_absence
        self.write({'state': 'confirm'})

    
    def action_attsheet_approve_all(self):
        for rec in self:
            rec.action_attsheet_approve()
        ...
        for rec in self:
            for line in rec.att_sheet_line_ids:
                ...
                if rec.employee_id.contract_id:
                    rec.employee_id.contract_id.late = late
                    rec.create_payslip()
                    rec.create_payslip().compute_sheet()
    ...
    def _get_emp_leave_intervals(self, emp, start_datetime=None, end_datetime=None):
        ...
        for leave in leave_ids:
            ...
            if leave.holiday_status_id.is_mission:
                print('leave', leave.amount_region)
                no += 1
                tot += leave.amount_region
        self.no_travel += no
        self.tot_travel += tot
        print('self.tot_travel', self.tot_travel)
        return leaves
    ...
    def get_attendances_total(self):
        for rec in self:
            rec.get_attendances()

    
    def back_to_draft_total(self):
        for rec in self:
            rec.state = 'draft'
            ...
            for day in all_dates:
                ...
                if work_intervals:
                    ...
                    if attendance_intervals:
                        ...
                        att_line.create(values)
                att_sheet.calculate_late_in()
                # if abs_flag:
                #     abs_cnt+=1

    
    def calculate_late_in(self):
        for rec in self:
            times = 0
            time_late = rec.att_policy_id.late_hr_rule_id.time_late
            calender_id = rec.employee_id.contract_id.resource_calendar_id

            for line in rec.att_sheet_line_ids:
                if line.ac_sign_in and line.ac_sign_in >= time_late:
                    times += 1
                    if times == 1:
                        line.late_in = calender_id.hours_per_day * float(self.env['late.hr.rule.line'].sudo().search(
                            [('late_id', '=', rec.att_policy_id.id), ('counter', '=', '1')], limit=1).rate)
                    elif times == 2:
                        line.late_in = calender_id.hours_per_day * self.env['late.hr.rule.line'].sudo().search(
                            [('late_id', '=', rec.att_policy_id.id), ('counter', '=', '2')], limit=1).rate
                    elif times == 3:
                        line.late_in = calender_id.hours_per_day * self.env['late.hr.rule.line'].sudo().search(
                            [('late_id', '=', rec.att_policy_id.id), ('counter', '=', '3')], limit=1).rate
                    elif times >= 4:
                        line.late_in = calender_id.hours_per_day * self.env['late.hr.rule.line'].sudo().search(
                            [('late_id', '=', rec.att_policy_id.id), ('counter', '=', '4')], limit=1).rate
                    else:
                        line.late_in = 0
    ...
    def create_payslip(self):
        ...
        for att_sheet in self:
            ...
            absence = [{
                ...
                'contract_id': contract_id,
                'sequence': 35,
                'number_of_days': att_sheet.current_absence,
                'number_of_hours': att_sheet.tot_current_absence_hours,
            }]
            late = [{
                'name': "Late In",
                'code': 'LATE',
...
class attendance_sheet_line(models.Model):
    ...
    worked_hours = fields.Float("Worked Hours", readonly=True, compute='get_late')
    ac_sign_in = fields.Float("Actual sign in", readonly=False)
    ac_sign_out = fields.Float("Actual sign out", readonly=False)
    ...
    diff_time = fields.Float("Diff Time", help="Diffrence between the working time and attendance time(s) ",
                             readonly=True)
    act_late_in = fields.Float("Actual Late In", readonly=True, compute='get_late')
    ...
    def get_late(self):
        for rec in self:
            # rec.late_in = rec.late_in
            rec.act_late_in = rec.act_late_in
            ...
            # for rec in self:
            #     late = rec.ac_sign_in - rec.pl_sign_in
            #     if late > 0:
            #         rec.act_late_in = late
            #         if 1 > late >= 0.5:
            #             rec.late_in = 1
            #         elif 2 > late >= 1:
            #             rec.late_in = 2
...
class HrPayslip(models.Model):
    ...
    travel_allowance = fields.Float(string="Travel Allowance", required=False, )
