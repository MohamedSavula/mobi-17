...
class hr_attendance_policy(models.Model):
    ...
    late_rule_id = fields.Many2one(comodel_name="hr.late.rule",required=True,
                                   string="Late In Rule")
    late_hr_rule_id = fields.Many2one(comodel_name="late.hr.rule", string="New Late Rule", required=True, )
    ...
    def get_overtime(self):
        ...
        if self:
            ...
            we_ot_id = self.overtime_rule_ids.search([('type', '=', 'weekend'), ('id', 'in', overtime_ids.ids)],
                                                       order='id', limit=1)
            ph_ot_id = self.overtime_rule_ids.search([('type', '=', 'ph'), ('id', 'in', overtime_ids.ids)],
                                                       order='id', limit=1)
            if wd_ot_id:
        ...
        # print 'the overtime self is ' + str(res)
        return res
...
class late_hr_rule(models.Model):
    _name = 'late.hr.rule'
    _description='Late Hr Rule'

    name = fields.Char(string='name', required=True)
    time_late = fields.Float(string="Time Late",  required=True, )
    line_ids = fields.One2many(comodel_name='late.hr.rule.line', inverse_name='late_id', string='Late In Periods')


class late_hr_rule_line(models.Model):
    _name = 'late.hr.rule.line'
    _description='Late Hr Rule Lines'
    times = [
        ('1', 'First Time'),
        ('2', 'Second Time'),
        ('3', 'Third Time'),
    ...
    late_id = fields.Many2one(comodel_name='late.hr.rule', string='name')
    rate = fields.Float(string='Rate',required=True)
    counter = fields.Selection(string="Times", selection=times, required=True, )
