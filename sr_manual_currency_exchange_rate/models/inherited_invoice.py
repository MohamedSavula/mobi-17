...
class AccountMove(models.Model):
    ...
    def button_create_landed_costs(self):
        ...
        landed_costs = self.env['stock.landed.cost'].create({
            ...
            'cost_lines': [(0, 0, {
                ...
                'price_unit': l.price_subtotal / self.manual_currency_exchange_rate if self.manual_currency_exchange_rate_x else l.currency_id._convert(
    ...
    def action_post(self):
        for rec in self:
            if not rec.invoice_date and rec.move_type == 'in_invoice':
                raise UserError('You must add Bill Date')
    ...
    def get_manual_currency_exchange_rate(self):
        for rec in self:
            ...
            rec.invoice_line_ids._onchange_amount_currency()
    ...
    #         new_lines += new_line
    #     # new_lines._onchange_mark_recompute_taxes()_sync_invoice
    #
...
class AccountMoveLine(models.Model):
    ...
    def _get_fields_onchange_subtotal_model(self, price_subtotal, move_type, currency, company, date):
        ...
        if self.move_id.apply_manual_currency_exchange and self.currency_id == self.move_id.currency_id:
    ...
    def _onchange_uom_id(self):
        ...
        if self.move_id.apply_manual_currency_exchange and price_unit and self.currency_id == self.move_id.currency_id:
    ...
    def _onchange_product_id(self):
        for line in self:
            ...
            if line.move_id.apply_manual_currency_exchange and line.currency_id == line.move_id.currency_id:
                line.price_unit = (
    ...
    def _sync_invoice(self, container):
        ...
        for line in after:
            if (
                ...
                if line.move_id.apply_manual_currency_exchange and line.currency_id == line.move_id.currency_id :
                    balance = line.amount_currency / line.move_id.manual_currency_exchange_rate if line.move_id.manual_currency_exchange_rate != 0 else line.amount_currency
    ...
    def _onchange_amount_currency(self):
        for line in self:
            ...
            if round(balance, 1) != round(line.balance, 1) and balance:
                line.balance = balance

            # if line.debit != debit and debit:
            #     line.debit = debit
            #
            # if line.credit != credit and credit:
            #     line.credit = credit
