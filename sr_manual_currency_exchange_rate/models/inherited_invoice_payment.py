...
class AccountPayments(models.Model):
    ...
    @api.onchange('manual_currency_exchange_rate_x')
    @api.constrains('manual_currency_exchange_rate_x')
    def update_rate_in_journal(self):
        for rec in self:
            if rec.move_id:
                rec.move_id.apply_manual_currency_exchange = True
    ...
    def write(self, vals):
        ...
        self._synchronize_to_moves(set(['amount']))
        return res
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None, force_balance=None):
        ...
        if self.active_manual_currency_rate:
            if self.apply_manual_currency_exchange:
                liquidity_balance = liquidity_amount_currency / self.manual_currency_exchange_rate if self.manual_currency_exchange_rate != 0 else 0
                write_off_balance = write_off_amount_currency / self.manual_currency_exchange_rate if self.manual_currency_exchange_rate != 0 else 0
            else:
